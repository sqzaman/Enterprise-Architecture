...
public class App {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Query query = session.createQuery(
        		 "select f from Flight f join f.origin airport join f.airplane airplance where airplance.capacity > :capacity and airport.country = :country");
        ...
        query = session.createQuery("select distinct a from Airline a join a.flights fl join fl.airplane pl where pl.model= :model");//
        ...
        query = session.createQuery("select f from Flight f join f.airline al join f.airplane ap where ap.model=:model and al.name <> :name");
        ...
        query = session.createQuery(" select f from Flight f where f.departureDate = :departureDate and f.departureTime < :departureTime ");
		
        
        query.setParameter("departureDate", df.parse("08/07/2009"));
		query.setParameter("departureTime", tf.parse("12:00 pm"));
        flights = query.list();
        System.out.printf("%-9s%-31s%-31s\n", "Flight:", "Departs:",
                "Arrives:");
        for (Flight flight : flights) {
